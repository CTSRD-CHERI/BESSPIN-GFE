//
// Generated by Bluespec Compiler (build 67161ca2)
//
//
// Ports:
// Name                         I/O  size props
// RDY_server_reset_request_put   O     1 reg
// RDY_server_reset_response_get  O     1 reg
// RDY_set_addr_map               O     1 const
// slave_awready                  O     1 reg
// slave_wready                   O     1 reg
// slave_bvalid                   O     1 reg
// slave_bid                      O     7 reg
// slave_bresp                    O     2 reg
// slave_arready                  O     1 reg
// slave_rvalid                   O     1 reg
// slave_rid                      O     7 reg
// slave_rdata                    O    64 reg
// slave_rresp                    O     2 reg
// slave_rlast                    O     1 reg
// get_to_console_get             O     8 reg
// RDY_get_to_console_get         O     1 reg
// RDY_put_from_console_put       O     1 reg
// intr                           O     1
// CLK                            I     1 clock
// RST_N                          I     1 reset
// set_addr_map_addr_base         I    64 reg
// set_addr_map_addr_lim          I    64 reg
// slave_awvalid                  I     1
// slave_awid                     I     7 reg
// slave_awaddr                   I    64 reg
// slave_awlen                    I     8 reg
// slave_awsize                   I     3 reg
// slave_awburst                  I     2 reg
// slave_awlock                   I     1 reg
// slave_awcache                  I     4 reg
// slave_awprot                   I     3 reg
// slave_awqos                    I     4 reg
// slave_awregion                 I     4 reg
// slave_wvalid                   I     1
// slave_wdata                    I    64 reg
// slave_wstrb                    I     8 reg
// slave_wlast                    I     1 reg
// slave_bready                   I     1
// slave_arvalid                  I     1
// slave_arid                     I     7 reg
// slave_araddr                   I    64 reg
// slave_arlen                    I     8 reg
// slave_arsize                   I     3 reg
// slave_arburst                  I     2 reg
// slave_arlock                   I     1 reg
// slave_arcache                  I     4 reg
// slave_arprot                   I     3 reg
// slave_arqos                    I     4 reg
// slave_arregion                 I     4 reg
// slave_rready                   I     1
// put_from_console_put           I     8 reg
// EN_server_reset_request_put    I     1
// EN_server_reset_response_get   I     1
// EN_set_addr_map                I     1
// EN_put_from_console_put        I     1
// EN_get_to_console_get          I     1
//
// No combinational paths from inputs to outputs
//
//

`ifdef BSV_ASSIGNMENT_DELAY
`else
  `define BSV_ASSIGNMENT_DELAY
`endif

`ifdef BSV_POSITIVE_RESET
  `define BSV_RESET_VALUE 1'b1
  `define BSV_RESET_EDGE posedge
`else
  `define BSV_RESET_VALUE 1'b0
  `define BSV_RESET_EDGE negedge
`endif

module mkUART(CLK,
	      RST_N,

	      EN_server_reset_request_put,
	      RDY_server_reset_request_put,

	      EN_server_reset_response_get,
	      RDY_server_reset_response_get,

	      set_addr_map_addr_base,
	      set_addr_map_addr_lim,
	      EN_set_addr_map,
	      RDY_set_addr_map,

	      slave_awvalid,
	      slave_awid,
	      slave_awaddr,
	      slave_awlen,
	      slave_awsize,
	      slave_awburst,
	      slave_awlock,
	      slave_awcache,
	      slave_awprot,
	      slave_awqos,
	      slave_awregion,

	      slave_awready,

	      slave_wvalid,
	      slave_wdata,
	      slave_wstrb,
	      slave_wlast,

	      slave_wready,

	      slave_bvalid,

	      slave_bid,

	      slave_bresp,

	      slave_bready,

	      slave_arvalid,
	      slave_arid,
	      slave_araddr,
	      slave_arlen,
	      slave_arsize,
	      slave_arburst,
	      slave_arlock,
	      slave_arcache,
	      slave_arprot,
	      slave_arqos,
	      slave_arregion,

	      slave_arready,

	      slave_rvalid,

	      slave_rid,

	      slave_rdata,

	      slave_rresp,

	      slave_rlast,

	      slave_rready,

	      EN_get_to_console_get,
	      get_to_console_get,
	      RDY_get_to_console_get,

	      put_from_console_put,
	      EN_put_from_console_put,
	      RDY_put_from_console_put,

	      intr);
  input  CLK;
  input  RST_N;

  // action method server_reset_request_put
  input  EN_server_reset_request_put;
  output RDY_server_reset_request_put;

  // action method server_reset_response_get
  input  EN_server_reset_response_get;
  output RDY_server_reset_response_get;

  // action method set_addr_map
  input  [63 : 0] set_addr_map_addr_base;
  input  [63 : 0] set_addr_map_addr_lim;
  input  EN_set_addr_map;
  output RDY_set_addr_map;

  // action method slave_m_awvalid
  input  slave_awvalid;
  input  [6 : 0] slave_awid;
  input  [63 : 0] slave_awaddr;
  input  [7 : 0] slave_awlen;
  input  [2 : 0] slave_awsize;
  input  [1 : 0] slave_awburst;
  input  slave_awlock;
  input  [3 : 0] slave_awcache;
  input  [2 : 0] slave_awprot;
  input  [3 : 0] slave_awqos;
  input  [3 : 0] slave_awregion;

  // value method slave_m_awready
  output slave_awready;

  // action method slave_m_wvalid
  input  slave_wvalid;
  input  [63 : 0] slave_wdata;
  input  [7 : 0] slave_wstrb;
  input  slave_wlast;

  // value method slave_m_wready
  output slave_wready;

  // value method slave_m_bvalid
  output slave_bvalid;

  // value method slave_m_bid
  output [6 : 0] slave_bid;

  // value method slave_m_bresp
  output [1 : 0] slave_bresp;

  // value method slave_m_buser

  // action method slave_m_bready
  input  slave_bready;

  // action method slave_m_arvalid
  input  slave_arvalid;
  input  [6 : 0] slave_arid;
  input  [63 : 0] slave_araddr;
  input  [7 : 0] slave_arlen;
  input  [2 : 0] slave_arsize;
  input  [1 : 0] slave_arburst;
  input  slave_arlock;
  input  [3 : 0] slave_arcache;
  input  [2 : 0] slave_arprot;
  input  [3 : 0] slave_arqos;
  input  [3 : 0] slave_arregion;

  // value method slave_m_arready
  output slave_arready;

  // value method slave_m_rvalid
  output slave_rvalid;

  // value method slave_m_rid
  output [6 : 0] slave_rid;

  // value method slave_m_rdata
  output [63 : 0] slave_rdata;

  // value method slave_m_rresp
  output [1 : 0] slave_rresp;

  // value method slave_m_rlast
  output slave_rlast;

  // value method slave_m_ruser

  // action method slave_m_rready
  input  slave_rready;

  // actionvalue method get_to_console_get
  input  EN_get_to_console_get;
  output [7 : 0] get_to_console_get;
  output RDY_get_to_console_get;

  // action method put_from_console_put
  input  [7 : 0] put_from_console_put;
  input  EN_put_from_console_put;
  output RDY_put_from_console_put;

  // value method intr
  output intr;

  // signals for module outputs
  wire [63 : 0] slave_rdata;
  wire [7 : 0] get_to_console_get;
  wire [6 : 0] slave_bid, slave_rid;
  wire [1 : 0] slave_bresp, slave_rresp;
  wire RDY_get_to_console_get,
       RDY_put_from_console_put,
       RDY_server_reset_request_put,
       RDY_server_reset_response_get,
       RDY_set_addr_map,
       intr,
       slave_arready,
       slave_awready,
       slave_bvalid,
       slave_rlast,
       slave_rvalid,
       slave_wready;

  // register cfg_verbosity
  reg [7 : 0] cfg_verbosity;
  wire [7 : 0] cfg_verbosity$D_IN;
  wire cfg_verbosity$EN;

  // register rg_addr_base
  reg [63 : 0] rg_addr_base;
  wire [63 : 0] rg_addr_base$D_IN;
  wire rg_addr_base$EN;

  // register rg_addr_lim
  reg [63 : 0] rg_addr_lim;
  wire [63 : 0] rg_addr_lim$D_IN;
  wire rg_addr_lim$EN;

  // register rg_dll
  reg [7 : 0] rg_dll;
  wire [7 : 0] rg_dll$D_IN;
  wire rg_dll$EN;

  // register rg_dlm
  reg [7 : 0] rg_dlm;
  wire [7 : 0] rg_dlm$D_IN;
  wire rg_dlm$EN;

  // register rg_fcr
  reg [7 : 0] rg_fcr;
  wire [7 : 0] rg_fcr$D_IN;
  wire rg_fcr$EN;

  // register rg_ier
  reg [7 : 0] rg_ier;
  wire [7 : 0] rg_ier$D_IN;
  wire rg_ier$EN;

  // register rg_lcr
  reg [7 : 0] rg_lcr;
  wire [7 : 0] rg_lcr$D_IN;
  wire rg_lcr$EN;

  // register rg_lsr
  reg [7 : 0] rg_lsr;
  reg [7 : 0] rg_lsr$D_IN;
  wire rg_lsr$EN;

  // register rg_mcr
  reg [7 : 0] rg_mcr;
  wire [7 : 0] rg_mcr$D_IN;
  wire rg_mcr$EN;

  // register rg_msr
  reg [7 : 0] rg_msr;
  wire [7 : 0] rg_msr$D_IN;
  wire rg_msr$EN;

  // register rg_rbr
  reg [7 : 0] rg_rbr;
  wire [7 : 0] rg_rbr$D_IN;
  wire rg_rbr$EN;

  // register rg_scr
  reg [7 : 0] rg_scr;
  wire [7 : 0] rg_scr$D_IN;
  wire rg_scr$EN;

  // register rg_state
  reg rg_state;
  wire rg_state$D_IN, rg_state$EN;

  // register rg_thr
  reg [7 : 0] rg_thr;
  wire [7 : 0] rg_thr$D_IN;
  wire rg_thr$EN;

  // ports of submodule f_from_console
  wire [7 : 0] f_from_console$D_IN, f_from_console$D_OUT;
  wire f_from_console$CLR,
       f_from_console$DEQ,
       f_from_console$EMPTY_N,
       f_from_console$ENQ,
       f_from_console$FULL_N;

  // ports of submodule f_reset_reqs
  wire f_reset_reqs$CLR,
       f_reset_reqs$DEQ,
       f_reset_reqs$EMPTY_N,
       f_reset_reqs$ENQ,
       f_reset_reqs$FULL_N;

  // ports of submodule f_reset_rsps
  wire f_reset_rsps$CLR,
       f_reset_rsps$DEQ,
       f_reset_rsps$EMPTY_N,
       f_reset_rsps$ENQ,
       f_reset_rsps$FULL_N;

  // ports of submodule f_to_console
  wire [7 : 0] f_to_console$D_IN, f_to_console$D_OUT;
  wire f_to_console$CLR,
       f_to_console$DEQ,
       f_to_console$EMPTY_N,
       f_to_console$ENQ,
       f_to_console$FULL_N;

  // ports of submodule slave_xactor_f_rd_addr
  wire [99 : 0] slave_xactor_f_rd_addr$D_IN, slave_xactor_f_rd_addr$D_OUT;
  wire slave_xactor_f_rd_addr$CLR,
       slave_xactor_f_rd_addr$DEQ,
       slave_xactor_f_rd_addr$EMPTY_N,
       slave_xactor_f_rd_addr$ENQ,
       slave_xactor_f_rd_addr$FULL_N;

  // ports of submodule slave_xactor_f_rd_data
  wire [73 : 0] slave_xactor_f_rd_data$D_IN, slave_xactor_f_rd_data$D_OUT;
  wire slave_xactor_f_rd_data$CLR,
       slave_xactor_f_rd_data$DEQ,
       slave_xactor_f_rd_data$EMPTY_N,
       slave_xactor_f_rd_data$ENQ,
       slave_xactor_f_rd_data$FULL_N;

  // ports of submodule slave_xactor_f_wr_addr
  wire [99 : 0] slave_xactor_f_wr_addr$D_IN, slave_xactor_f_wr_addr$D_OUT;
  wire slave_xactor_f_wr_addr$CLR,
       slave_xactor_f_wr_addr$DEQ,
       slave_xactor_f_wr_addr$EMPTY_N,
       slave_xactor_f_wr_addr$ENQ,
       slave_xactor_f_wr_addr$FULL_N;

  // ports of submodule slave_xactor_f_wr_data
  wire [72 : 0] slave_xactor_f_wr_data$D_IN, slave_xactor_f_wr_data$D_OUT;
  wire slave_xactor_f_wr_data$CLR,
       slave_xactor_f_wr_data$DEQ,
       slave_xactor_f_wr_data$EMPTY_N,
       slave_xactor_f_wr_data$ENQ,
       slave_xactor_f_wr_data$FULL_N;

  // ports of submodule slave_xactor_f_wr_resp
  wire [8 : 0] slave_xactor_f_wr_resp$D_IN, slave_xactor_f_wr_resp$D_OUT;
  wire slave_xactor_f_wr_resp$CLR,
       slave_xactor_f_wr_resp$DEQ,
       slave_xactor_f_wr_resp$EMPTY_N,
       slave_xactor_f_wr_resp$ENQ,
       slave_xactor_f_wr_resp$FULL_N;

  // rule scheduling signals
  wire WILL_FIRE_RL_rl_process_rd_req,
       WILL_FIRE_RL_rl_process_wr_req,
       WILL_FIRE_RL_rl_receive,
       WILL_FIRE_RL_rl_reset;

  // inputs to muxes for submodule ports
  wire [7 : 0] MUX_rg_dll$write_1__VAL_2, MUX_rg_lsr$write_1__VAL_3;
  wire MUX_rg_fcr$write_1__SEL_2,
       MUX_rg_lcr$write_1__SEL_2,
       MUX_rg_lsr$write_1__SEL_3,
       MUX_rg_mcr$write_1__SEL_2,
       MUX_rg_scr$write_1__SEL_2;

  // declarations used by system tasks
  // synopsys translate_off
  reg [31 : 0] v__h2602;
  reg [31 : 0] v__h2241;
  reg [31 : 0] v__h2061;
  reg [31 : 0] v__h3121;
  reg [31 : 0] v__h3495;
  reg [31 : 0] v__h4389;
  reg [31 : 0] v__h3725;
  reg [31 : 0] v__h4708;
  reg [31 : 0] v__h5175;
  reg [31 : 0] v__h5281;
  reg [31 : 0] v__h1852;
  reg [31 : 0] v__h1846;
  reg [31 : 0] v__h2055;
  reg [31 : 0] v__h2235;
  reg [31 : 0] v__h2596;
  reg [31 : 0] v__h3115;
  reg [31 : 0] v__h3489;
  reg [31 : 0] v__h3719;
  reg [31 : 0] v__h4383;
  reg [31 : 0] v__h4702;
  reg [31 : 0] v__h5169;
  reg [31 : 0] v__h5275;
  // synopsys translate_on

  // remaining internal signals
  reg [7 : 0] y_avValue_snd_fst__h2977, y_avValue_snd_snd__h2964;
  wire [63 : 0] IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q2,
		IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153,
		byte_addr___1__h3429,
		byte_addr__h3363,
		rdr_rdata__h2901,
		slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17,
		wdata___1__h3428;
  wire [7 : 0] IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1,
	       fn_iir__h1349,
	       new_lsr__h4946,
	       wstrb___1__h3427,
	       x__h2946,
	       x__h3056,
	       y_avValue_snd_fst__h2951,
	       y_avValue_snd_fst__h2957,
	       y_avValue_snd_fst__h2963,
	       y_avValue_snd_fst__h2971,
	       y_avValue_snd_snd__h2952,
	       y_avValue_snd_snd__h2958;
  wire [1 : 0] rdr_rresp__h2902,
	       v__h3375,
	       v__h3671,
	       v__h3876,
	       y_avValue_fst__h2849,
	       y_avValue_fst__h2865;
  wire IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d197,
       IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d204,
       IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d208,
       IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d211,
       NOT_IF_slave_xactor_f_wr_data_first__44_BITS_8_ETC___d254,
       NOT_IF_slave_xactor_f_wr_data_first__44_BITS_8_ETC___d265,
       NOT_cfg_verbosity_read_ULE_1_37___d138,
       slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d29,
       slave_xactor_f_wr_resp_i_notFull__43_AND_NOT_I_ETC___d174;

  // action method server_reset_request_put
  assign RDY_server_reset_request_put = f_reset_reqs$FULL_N ;

  // action method server_reset_response_get
  assign RDY_server_reset_response_get = f_reset_rsps$EMPTY_N ;

  // action method set_addr_map
  assign RDY_set_addr_map = 1'd1 ;

  // value method slave_m_awready
  assign slave_awready = slave_xactor_f_wr_addr$FULL_N ;

  // value method slave_m_wready
  assign slave_wready = slave_xactor_f_wr_data$FULL_N ;

  // value method slave_m_bvalid
  assign slave_bvalid = slave_xactor_f_wr_resp$EMPTY_N ;

  // value method slave_m_bid
  assign slave_bid = slave_xactor_f_wr_resp$D_OUT[8:2] ;

  // value method slave_m_bresp
  assign slave_bresp = slave_xactor_f_wr_resp$D_OUT[1:0] ;

  // value method slave_m_arready
  assign slave_arready = slave_xactor_f_rd_addr$FULL_N ;

  // value method slave_m_rvalid
  assign slave_rvalid = slave_xactor_f_rd_data$EMPTY_N ;

  // value method slave_m_rid
  assign slave_rid = slave_xactor_f_rd_data$D_OUT[73:67] ;

  // value method slave_m_rdata
  assign slave_rdata = slave_xactor_f_rd_data$D_OUT[66:3] ;

  // value method slave_m_rresp
  assign slave_rresp = slave_xactor_f_rd_data$D_OUT[2:1] ;

  // value method slave_m_rlast
  assign slave_rlast = slave_xactor_f_rd_data$D_OUT[0] ;

  // actionvalue method get_to_console_get
  assign get_to_console_get = f_to_console$D_OUT ;
  assign RDY_get_to_console_get = f_to_console$EMPTY_N ;

  // action method put_from_console_put
  assign RDY_put_from_console_put = f_from_console$FULL_N ;

  // value method intr
  assign intr = !fn_iir__h1349[0] ;

  // submodule f_from_console
  FIFO2 #(.width(32'd8), .guarded(1'd1)) f_from_console(.RST(RST_N),
							.CLK(CLK),
							.D_IN(f_from_console$D_IN),
							.ENQ(f_from_console$ENQ),
							.DEQ(f_from_console$DEQ),
							.CLR(f_from_console$CLR),
							.D_OUT(f_from_console$D_OUT),
							.FULL_N(f_from_console$FULL_N),
							.EMPTY_N(f_from_console$EMPTY_N));

  // submodule f_reset_reqs
  FIFO20 #(.guarded(1'd1)) f_reset_reqs(.RST(RST_N),
					.CLK(CLK),
					.ENQ(f_reset_reqs$ENQ),
					.DEQ(f_reset_reqs$DEQ),
					.CLR(f_reset_reqs$CLR),
					.FULL_N(f_reset_reqs$FULL_N),
					.EMPTY_N(f_reset_reqs$EMPTY_N));

  // submodule f_reset_rsps
  FIFO20 #(.guarded(1'd1)) f_reset_rsps(.RST(RST_N),
					.CLK(CLK),
					.ENQ(f_reset_rsps$ENQ),
					.DEQ(f_reset_rsps$DEQ),
					.CLR(f_reset_rsps$CLR),
					.FULL_N(f_reset_rsps$FULL_N),
					.EMPTY_N(f_reset_rsps$EMPTY_N));

  // submodule f_to_console
  FIFO2 #(.width(32'd8), .guarded(1'd1)) f_to_console(.RST(RST_N),
						      .CLK(CLK),
						      .D_IN(f_to_console$D_IN),
						      .ENQ(f_to_console$ENQ),
						      .DEQ(f_to_console$DEQ),
						      .CLR(f_to_console$CLR),
						      .D_OUT(f_to_console$D_OUT),
						      .FULL_N(f_to_console$FULL_N),
						      .EMPTY_N(f_to_console$EMPTY_N));

  // submodule slave_xactor_f_rd_addr
  FIFO2 #(.width(32'd100), .guarded(1'd1)) slave_xactor_f_rd_addr(.RST(RST_N),
								  .CLK(CLK),
								  .D_IN(slave_xactor_f_rd_addr$D_IN),
								  .ENQ(slave_xactor_f_rd_addr$ENQ),
								  .DEQ(slave_xactor_f_rd_addr$DEQ),
								  .CLR(slave_xactor_f_rd_addr$CLR),
								  .D_OUT(slave_xactor_f_rd_addr$D_OUT),
								  .FULL_N(slave_xactor_f_rd_addr$FULL_N),
								  .EMPTY_N(slave_xactor_f_rd_addr$EMPTY_N));

  // submodule slave_xactor_f_rd_data
  FIFO2 #(.width(32'd74), .guarded(1'd1)) slave_xactor_f_rd_data(.RST(RST_N),
								 .CLK(CLK),
								 .D_IN(slave_xactor_f_rd_data$D_IN),
								 .ENQ(slave_xactor_f_rd_data$ENQ),
								 .DEQ(slave_xactor_f_rd_data$DEQ),
								 .CLR(slave_xactor_f_rd_data$CLR),
								 .D_OUT(slave_xactor_f_rd_data$D_OUT),
								 .FULL_N(slave_xactor_f_rd_data$FULL_N),
								 .EMPTY_N(slave_xactor_f_rd_data$EMPTY_N));

  // submodule slave_xactor_f_wr_addr
  FIFO2 #(.width(32'd100), .guarded(1'd1)) slave_xactor_f_wr_addr(.RST(RST_N),
								  .CLK(CLK),
								  .D_IN(slave_xactor_f_wr_addr$D_IN),
								  .ENQ(slave_xactor_f_wr_addr$ENQ),
								  .DEQ(slave_xactor_f_wr_addr$DEQ),
								  .CLR(slave_xactor_f_wr_addr$CLR),
								  .D_OUT(slave_xactor_f_wr_addr$D_OUT),
								  .FULL_N(slave_xactor_f_wr_addr$FULL_N),
								  .EMPTY_N(slave_xactor_f_wr_addr$EMPTY_N));

  // submodule slave_xactor_f_wr_data
  FIFO2 #(.width(32'd73), .guarded(1'd1)) slave_xactor_f_wr_data(.RST(RST_N),
								 .CLK(CLK),
								 .D_IN(slave_xactor_f_wr_data$D_IN),
								 .ENQ(slave_xactor_f_wr_data$ENQ),
								 .DEQ(slave_xactor_f_wr_data$DEQ),
								 .CLR(slave_xactor_f_wr_data$CLR),
								 .D_OUT(slave_xactor_f_wr_data$D_OUT),
								 .FULL_N(slave_xactor_f_wr_data$FULL_N),
								 .EMPTY_N(slave_xactor_f_wr_data$EMPTY_N));

  // submodule slave_xactor_f_wr_resp
  FIFO2 #(.width(32'd9), .guarded(1'd1)) slave_xactor_f_wr_resp(.RST(RST_N),
								.CLK(CLK),
								.D_IN(slave_xactor_f_wr_resp$D_IN),
								.ENQ(slave_xactor_f_wr_resp$ENQ),
								.DEQ(slave_xactor_f_wr_resp$DEQ),
								.CLR(slave_xactor_f_wr_resp$CLR),
								.D_OUT(slave_xactor_f_wr_resp$D_OUT),
								.FULL_N(slave_xactor_f_wr_resp$FULL_N),
								.EMPTY_N(slave_xactor_f_wr_resp$EMPTY_N));

  // rule RL_rl_process_rd_req
  assign WILL_FIRE_RL_rl_process_rd_req =
	     slave_xactor_f_rd_addr$EMPTY_N &&
	     slave_xactor_f_rd_data$FULL_N &&
	     rg_state ;

  // rule RL_rl_process_wr_req
  assign WILL_FIRE_RL_rl_process_wr_req =
	     slave_xactor_f_wr_addr$EMPTY_N &&
	     slave_xactor_f_wr_data$EMPTY_N &&
	     slave_xactor_f_wr_resp_i_notFull__43_AND_NOT_I_ETC___d174 &&
	     rg_state ;

  // rule RL_rl_receive
  assign WILL_FIRE_RL_rl_receive =
	     f_from_console$EMPTY_N && !rg_lsr[0] &&
	     !WILL_FIRE_RL_rl_process_rd_req ;

  // rule RL_rl_reset
  assign WILL_FIRE_RL_rl_reset = f_reset_reqs$EMPTY_N && f_reset_rsps$FULL_N ;

  // inputs to muxes for submodule ports
  assign MUX_rg_fcr$write_1__SEL_2 =
	     WILL_FIRE_RL_rl_process_wr_req &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	     2'd0 &&
	     IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	     3'd0 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] ==
	     3'h2 ;
  assign MUX_rg_lcr$write_1__SEL_2 =
	     WILL_FIRE_RL_rl_process_wr_req &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	     2'd0 &&
	     IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	     3'd0 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] ==
	     3'h3 ;
  assign MUX_rg_lsr$write_1__SEL_3 =
	     WILL_FIRE_RL_rl_process_rd_req &&
	     slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d29 ;
  assign MUX_rg_mcr$write_1__SEL_2 =
	     WILL_FIRE_RL_rl_process_wr_req &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	     2'd0 &&
	     IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	     3'd0 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] ==
	     3'h4 ;
  assign MUX_rg_scr$write_1__SEL_2 =
	     WILL_FIRE_RL_rl_process_wr_req &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	     2'd0 &&
	     IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	     3'd0 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] ==
	     3'h7 ;
  assign MUX_rg_dll$write_1__VAL_2 =
	     IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q2[7:0] ;
  assign MUX_rg_lsr$write_1__VAL_3 = { rg_lsr[7:1], 1'd0 } ;

  // register cfg_verbosity
  assign cfg_verbosity$D_IN = 8'h0 ;
  assign cfg_verbosity$EN = 1'b0 ;

  // register rg_addr_base
  assign rg_addr_base$D_IN = set_addr_map_addr_base ;
  assign rg_addr_base$EN = EN_set_addr_map ;

  // register rg_addr_lim
  assign rg_addr_lim$D_IN = set_addr_map_addr_lim ;
  assign rg_addr_lim$EN = EN_set_addr_map ;

  // register rg_dll
  assign rg_dll$D_IN =
	     WILL_FIRE_RL_rl_reset ?
	       8'd0 :
	       IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q2[7:0] ;
  assign rg_dll$EN =
	     WILL_FIRE_RL_rl_process_wr_req &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d204 ||
	     WILL_FIRE_RL_rl_reset ;

  // register rg_dlm
  assign rg_dlm$D_IN =
	     WILL_FIRE_RL_rl_reset ?
	       8'd0 :
	       IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q2[7:0] ;
  assign rg_dlm$EN =
	     WILL_FIRE_RL_rl_process_wr_req &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d211 ||
	     WILL_FIRE_RL_rl_reset ;

  // register rg_fcr
  assign rg_fcr$D_IN =
	     WILL_FIRE_RL_rl_reset ?
	       8'd0 :
	       IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q2[7:0] ;
  assign rg_fcr$EN = MUX_rg_fcr$write_1__SEL_2 || WILL_FIRE_RL_rl_reset ;

  // register rg_ier
  assign rg_ier$D_IN =
	     WILL_FIRE_RL_rl_reset ?
	       8'd0 :
	       IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q2[7:0] ;
  assign rg_ier$EN =
	     WILL_FIRE_RL_rl_process_wr_req &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d208 ||
	     WILL_FIRE_RL_rl_reset ;

  // register rg_lcr
  assign rg_lcr$D_IN =
	     WILL_FIRE_RL_rl_reset ?
	       8'd0 :
	       IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q2[7:0] ;
  assign rg_lcr$EN = MUX_rg_lcr$write_1__SEL_2 || WILL_FIRE_RL_rl_reset ;

  // register rg_lsr
  always@(WILL_FIRE_RL_rl_reset or
	  WILL_FIRE_RL_rl_receive or
	  new_lsr__h4946 or
	  MUX_rg_lsr$write_1__SEL_3 or MUX_rg_lsr$write_1__VAL_3)
  case (1'b1)
    WILL_FIRE_RL_rl_reset: rg_lsr$D_IN = 8'd96;
    WILL_FIRE_RL_rl_receive: rg_lsr$D_IN = new_lsr__h4946;
    MUX_rg_lsr$write_1__SEL_3: rg_lsr$D_IN = MUX_rg_lsr$write_1__VAL_3;
    default: rg_lsr$D_IN = 8'b10101010 /* unspecified value */ ;
  endcase
  assign rg_lsr$EN =
	     WILL_FIRE_RL_rl_process_rd_req &&
	     slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d29 ||
	     WILL_FIRE_RL_rl_receive ||
	     WILL_FIRE_RL_rl_reset ;

  // register rg_mcr
  assign rg_mcr$D_IN =
	     WILL_FIRE_RL_rl_reset ?
	       8'd0 :
	       IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q2[7:0] ;
  assign rg_mcr$EN = MUX_rg_mcr$write_1__SEL_2 || WILL_FIRE_RL_rl_reset ;

  // register rg_msr
  assign rg_msr$D_IN = 8'd0 ;
  assign rg_msr$EN = WILL_FIRE_RL_rl_reset ;

  // register rg_rbr
  assign rg_rbr$D_IN = f_from_console$D_OUT ;
  assign rg_rbr$EN = WILL_FIRE_RL_rl_receive ;

  // register rg_scr
  assign rg_scr$D_IN =
	     WILL_FIRE_RL_rl_reset ?
	       8'd0 :
	       IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q2[7:0] ;
  assign rg_scr$EN = MUX_rg_scr$write_1__SEL_2 || WILL_FIRE_RL_rl_reset ;

  // register rg_state
  assign rg_state$D_IN = 1'd1 ;
  assign rg_state$EN = f_reset_reqs$EMPTY_N && f_reset_rsps$FULL_N ;

  // register rg_thr
  assign rg_thr$D_IN = MUX_rg_dll$write_1__VAL_2 ;
  assign rg_thr$EN =
	     WILL_FIRE_RL_rl_process_wr_req &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d197 ;

  // submodule f_from_console
  assign f_from_console$D_IN = put_from_console_put ;
  assign f_from_console$ENQ = EN_put_from_console_put ;
  assign f_from_console$DEQ = WILL_FIRE_RL_rl_receive ;
  assign f_from_console$CLR = 1'b0 ;

  // submodule f_reset_reqs
  assign f_reset_reqs$ENQ = EN_server_reset_request_put ;
  assign f_reset_reqs$DEQ = f_reset_reqs$EMPTY_N && f_reset_rsps$FULL_N ;
  assign f_reset_reqs$CLR = 1'b0 ;

  // submodule f_reset_rsps
  assign f_reset_rsps$ENQ = f_reset_reqs$EMPTY_N && f_reset_rsps$FULL_N ;
  assign f_reset_rsps$DEQ = EN_server_reset_response_get ;
  assign f_reset_rsps$CLR = 1'b0 ;

  // submodule f_to_console
  assign f_to_console$D_IN = MUX_rg_dll$write_1__VAL_2 ;
  assign f_to_console$ENQ =
	     WILL_FIRE_RL_rl_process_wr_req &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d197 ;
  assign f_to_console$DEQ = EN_get_to_console_get ;
  assign f_to_console$CLR = 1'b0 ;

  // submodule slave_xactor_f_rd_addr
  assign slave_xactor_f_rd_addr$D_IN =
	     { slave_arid,
	       slave_araddr,
	       slave_arlen,
	       slave_arsize,
	       slave_arburst,
	       slave_arlock,
	       slave_arcache,
	       slave_arprot,
	       slave_arqos,
	       slave_arregion } ;
  assign slave_xactor_f_rd_addr$ENQ =
	     slave_arvalid && slave_xactor_f_rd_addr$FULL_N ;
  assign slave_xactor_f_rd_addr$DEQ = WILL_FIRE_RL_rl_process_rd_req ;
  assign slave_xactor_f_rd_addr$CLR =
	     f_reset_reqs$EMPTY_N && f_reset_rsps$FULL_N ;

  // submodule slave_xactor_f_rd_data
  assign slave_xactor_f_rd_data$D_IN =
	     { slave_xactor_f_rd_addr$D_OUT[99:93],
	       rdr_rdata__h2901,
	       rdr_rresp__h2902,
	       1'd1 } ;
  assign slave_xactor_f_rd_data$ENQ = WILL_FIRE_RL_rl_process_rd_req ;
  assign slave_xactor_f_rd_data$DEQ =
	     slave_rready && slave_xactor_f_rd_data$EMPTY_N ;
  assign slave_xactor_f_rd_data$CLR =
	     f_reset_reqs$EMPTY_N && f_reset_rsps$FULL_N ;

  // submodule slave_xactor_f_wr_addr
  assign slave_xactor_f_wr_addr$D_IN =
	     { slave_awid,
	       slave_awaddr,
	       slave_awlen,
	       slave_awsize,
	       slave_awburst,
	       slave_awlock,
	       slave_awcache,
	       slave_awprot,
	       slave_awqos,
	       slave_awregion } ;
  assign slave_xactor_f_wr_addr$ENQ =
	     slave_awvalid && slave_xactor_f_wr_addr$FULL_N ;
  assign slave_xactor_f_wr_addr$DEQ = WILL_FIRE_RL_rl_process_wr_req ;
  assign slave_xactor_f_wr_addr$CLR =
	     f_reset_reqs$EMPTY_N && f_reset_rsps$FULL_N ;

  // submodule slave_xactor_f_wr_data
  assign slave_xactor_f_wr_data$D_IN =
	     { slave_wdata, slave_wstrb, slave_wlast } ;
  assign slave_xactor_f_wr_data$ENQ =
	     slave_wvalid && slave_xactor_f_wr_data$FULL_N ;
  assign slave_xactor_f_wr_data$DEQ = WILL_FIRE_RL_rl_process_wr_req ;
  assign slave_xactor_f_wr_data$CLR =
	     f_reset_reqs$EMPTY_N && f_reset_rsps$FULL_N ;

  // submodule slave_xactor_f_wr_resp
  assign slave_xactor_f_wr_resp$D_IN =
	     { slave_xactor_f_wr_addr$D_OUT[99:93], v__h3375 } ;
  assign slave_xactor_f_wr_resp$ENQ = WILL_FIRE_RL_rl_process_wr_req ;
  assign slave_xactor_f_wr_resp$DEQ =
	     slave_bready && slave_xactor_f_wr_resp$EMPTY_N ;
  assign slave_xactor_f_wr_resp$CLR =
	     f_reset_reqs$EMPTY_N && f_reset_rsps$FULL_N ;

  // remaining internal signals
  assign IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1 =
	     (slave_xactor_f_wr_data$D_OUT[8:5] == 4'd0) ?
	       slave_xactor_f_wr_data$D_OUT[8:1] :
	       wstrb___1__h3427 ;
  assign IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q2 =
	     (slave_xactor_f_wr_data$D_OUT[8:5] == 4'd0) ?
	       slave_xactor_f_wr_data$D_OUT[72:9] :
	       wdata___1__h3428 ;
  assign IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153 =
	     (slave_xactor_f_wr_data$D_OUT[8:5] == 4'd0) ?
	       byte_addr__h3363 :
	       byte_addr___1__h3429 ;
  assign IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d197 =
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	     2'd0 &&
	     IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	     3'd0 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] ==
	     3'h0 &&
	     !rg_lcr[7] ;
  assign IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d204 =
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	     2'd0 &&
	     IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	     3'd0 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] ==
	     3'h0 &&
	     rg_lcr[7] ;
  assign IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d208 =
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	     2'd0 &&
	     IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	     3'd0 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] ==
	     3'h1 &&
	     !rg_lcr[7] ;
  assign IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d211 =
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	     2'd0 &&
	     IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	     3'd0 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] ==
	     3'h1 &&
	     rg_lcr[7] ;
  assign NOT_IF_slave_xactor_f_wr_data_first__44_BITS_8_ETC___d254 =
	     (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	      3'h1 ||
	      !rg_lcr[7]) &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h2 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h3 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h4 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h5 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h6 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h7 &&
	     slave_xactor_f_wr_data$D_OUT[0] ;
  assign NOT_IF_slave_xactor_f_wr_data_first__44_BITS_8_ETC___d265 =
	     (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	      3'h1 ||
	      !rg_lcr[7]) &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h2 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h3 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h4 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h5 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h6 &&
	     IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	     3'h7 &&
	     !slave_xactor_f_wr_data$D_OUT[0] ;
  assign NOT_cfg_verbosity_read_ULE_1_37___d138 = cfg_verbosity > 8'd1 ;
  assign byte_addr___1__h3429 =
	     { byte_addr__h3363[63:3], 1'd1, byte_addr__h3363[1:0] } ;
  assign byte_addr__h3363 =
	     slave_xactor_f_wr_addr$D_OUT[92:29] - rg_addr_base ;
  assign fn_iir__h1349 =
	     (rg_ier[0] && rg_lsr[0]) ? 8'h04 : (rg_ier[1] ? 8'h02 : 8'd0) ;
  assign new_lsr__h4946 = { rg_lsr[7:1], 1'd1 } ;
  assign rdr_rdata__h2901 =
	     { 24'd0,
	       slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[2] ?
		 x__h2946 :
		 x__h3056,
	       slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[2] ?
		 32'd0 :
		 { 24'd0, x__h2946 } } ;
  assign rdr_rresp__h2902 =
	     (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	      2'd0) ?
	       y_avValue_fst__h2865 :
	       2'b10 ;
  assign slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17 =
	     slave_xactor_f_rd_addr$D_OUT[92:29] - rg_addr_base ;
  assign slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d29 =
	     slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	     2'd0 &&
	     slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] ==
	     3'd0 &&
	     slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[4:2] ==
	     3'h0 &&
	     !rg_lcr[7] ;
  assign slave_xactor_f_wr_resp_i_notFull__43_AND_NOT_I_ETC___d174 =
	     slave_xactor_f_wr_resp$FULL_N &&
	     (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	      2'd0 ||
	      !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] ||
	      IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	      3'd0 ||
	      IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	      3'h0 ||
	      rg_lcr[7] ||
	      f_to_console$FULL_N) ;
  assign v__h3375 =
	     (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	      2'd0 ||
	      !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]) ?
	       2'b10 :
	       v__h3671 ;
  assign v__h3671 =
	     (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	      3'd0) ?
	       v__h3876 :
	       2'b11 ;
  assign v__h3876 = y_avValue_fst__h2849 ;
  assign wdata___1__h3428 = { 32'd0, slave_xactor_f_wr_data$D_OUT[72:41] } ;
  assign wstrb___1__h3427 = { 4'd0, slave_xactor_f_wr_data$D_OUT[8:5] } ;
  assign x__h2946 =
	     (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	      2'd0 ||
	      slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	      3'd0) ?
	       8'd0 :
	       y_avValue_snd_fst__h2951 ;
  assign x__h3056 =
	     (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	      2'd0 ||
	      slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	      3'd0) ?
	       8'd0 :
	       y_avValue_snd_snd__h2952 ;
  assign y_avValue_fst__h2849 = 2'b0 ;
  assign y_avValue_fst__h2865 =
	     (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] ==
	      3'd0) ?
	       y_avValue_fst__h2849 :
	       2'b11 ;
  assign y_avValue_snd_fst__h2951 =
	     (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[4:2] ==
	      3'h0 &&
	      !rg_lcr[7]) ?
	       rg_rbr :
	       y_avValue_snd_fst__h2957 ;
  assign y_avValue_snd_fst__h2957 =
	     (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[4:2] ==
	      3'h0 &&
	      rg_lcr[7]) ?
	       rg_dll :
	       y_avValue_snd_fst__h2963 ;
  assign y_avValue_snd_fst__h2963 =
	     (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[4:2] ==
	      3'h1 &&
	      !rg_lcr[7]) ?
	       rg_ier :
	       y_avValue_snd_fst__h2971 ;
  assign y_avValue_snd_fst__h2971 =
	     (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[4:2] ==
	      3'h1 &&
	      rg_lcr[7]) ?
	       rg_dlm :
	       y_avValue_snd_fst__h2977 ;
  assign y_avValue_snd_snd__h2952 =
	     (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[4:2] ==
	      3'h0 &&
	      !rg_lcr[7]) ?
	       rg_ier :
	       y_avValue_snd_snd__h2958 ;
  assign y_avValue_snd_snd__h2958 =
	     (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[4:2] ==
	      3'h0 &&
	      rg_lcr[7]) ?
	       rg_dlm :
	       y_avValue_snd_snd__h2964 ;
  always@(slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17 or
	  rg_lcr or rg_lsr or rg_scr)
  begin
    case (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[4:2])
      3'h1, 3'h3, 3'h5: y_avValue_snd_snd__h2964 = 8'd0;
      3'h2: y_avValue_snd_snd__h2964 = rg_lcr;
      3'h4: y_avValue_snd_snd__h2964 = rg_lsr;
      3'h6: y_avValue_snd_snd__h2964 = rg_scr;
      default: y_avValue_snd_snd__h2964 = 8'd0;
    endcase
  end
  always@(slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17 or
	  fn_iir__h1349 or rg_lcr or rg_mcr or rg_lsr or rg_msr or rg_scr)
  begin
    case (slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[4:2])
      3'h2: y_avValue_snd_fst__h2977 = fn_iir__h1349;
      3'h3: y_avValue_snd_fst__h2977 = rg_lcr;
      3'h4: y_avValue_snd_fst__h2977 = rg_mcr;
      3'h5: y_avValue_snd_fst__h2977 = rg_lsr;
      3'h6: y_avValue_snd_fst__h2977 = rg_msr;
      3'h7: y_avValue_snd_fst__h2977 = rg_scr;
      default: y_avValue_snd_fst__h2977 = 8'd0;
    endcase
  end

  // handling of inlined registers

  always@(posedge CLK)
  begin
    if (RST_N == `BSV_RESET_VALUE)
      begin
        cfg_verbosity <= `BSV_ASSIGNMENT_DELAY 8'd0;
	rg_dll <= `BSV_ASSIGNMENT_DELAY 8'd0;
	rg_dlm <= `BSV_ASSIGNMENT_DELAY 8'd0;
	rg_fcr <= `BSV_ASSIGNMENT_DELAY 8'd0;
	rg_ier <= `BSV_ASSIGNMENT_DELAY 8'd0;
	rg_lcr <= `BSV_ASSIGNMENT_DELAY 8'd0;
	rg_lsr <= `BSV_ASSIGNMENT_DELAY 8'd96;
	rg_mcr <= `BSV_ASSIGNMENT_DELAY 8'd0;
	rg_msr <= `BSV_ASSIGNMENT_DELAY 8'd0;
	rg_scr <= `BSV_ASSIGNMENT_DELAY 8'd0;
	rg_state <= `BSV_ASSIGNMENT_DELAY 1'd0;
      end
    else
      begin
        if (cfg_verbosity$EN)
	  cfg_verbosity <= `BSV_ASSIGNMENT_DELAY cfg_verbosity$D_IN;
	if (rg_dll$EN) rg_dll <= `BSV_ASSIGNMENT_DELAY rg_dll$D_IN;
	if (rg_dlm$EN) rg_dlm <= `BSV_ASSIGNMENT_DELAY rg_dlm$D_IN;
	if (rg_fcr$EN) rg_fcr <= `BSV_ASSIGNMENT_DELAY rg_fcr$D_IN;
	if (rg_ier$EN) rg_ier <= `BSV_ASSIGNMENT_DELAY rg_ier$D_IN;
	if (rg_lcr$EN) rg_lcr <= `BSV_ASSIGNMENT_DELAY rg_lcr$D_IN;
	if (rg_lsr$EN) rg_lsr <= `BSV_ASSIGNMENT_DELAY rg_lsr$D_IN;
	if (rg_mcr$EN) rg_mcr <= `BSV_ASSIGNMENT_DELAY rg_mcr$D_IN;
	if (rg_msr$EN) rg_msr <= `BSV_ASSIGNMENT_DELAY rg_msr$D_IN;
	if (rg_scr$EN) rg_scr <= `BSV_ASSIGNMENT_DELAY rg_scr$D_IN;
	if (rg_state$EN) rg_state <= `BSV_ASSIGNMENT_DELAY rg_state$D_IN;
      end
    if (rg_addr_base$EN)
      rg_addr_base <= `BSV_ASSIGNMENT_DELAY rg_addr_base$D_IN;
    if (rg_addr_lim$EN) rg_addr_lim <= `BSV_ASSIGNMENT_DELAY rg_addr_lim$D_IN;
    if (rg_rbr$EN) rg_rbr <= `BSV_ASSIGNMENT_DELAY rg_rbr$D_IN;
    if (rg_thr$EN) rg_thr <= `BSV_ASSIGNMENT_DELAY rg_thr$D_IN;
  end

  // synopsys translate_off
  `ifdef BSV_NO_INITIAL_BLOCKS
  `else // not BSV_NO_INITIAL_BLOCKS
  initial
  begin
    cfg_verbosity = 8'hAA;
    rg_addr_base = 64'hAAAAAAAAAAAAAAAA;
    rg_addr_lim = 64'hAAAAAAAAAAAAAAAA;
    rg_dll = 8'hAA;
    rg_dlm = 8'hAA;
    rg_fcr = 8'hAA;
    rg_ier = 8'hAA;
    rg_lcr = 8'hAA;
    rg_lsr = 8'hAA;
    rg_mcr = 8'hAA;
    rg_msr = 8'hAA;
    rg_rbr = 8'hAA;
    rg_scr = 8'hAA;
    rg_state = 1'h0;
    rg_thr = 8'hAA;
  end
  `endif // BSV_NO_INITIAL_BLOCKS
  // synopsys translate_on

  // handling of system tasks

  // synopsys translate_off
  always@(negedge CLK)
  begin
    #0;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	begin
	  v__h2602 = $stime;
	  #0;
	end
    v__h2596 = v__h2602 / 32'd10;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$display("%0d: ERROR: UART.rl_process_rd_req: unrecognized addr",
		 v__h2596);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("AXI4_Rd_Addr { ", "arid: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[99:93]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write(", ", "araddr: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[92:29]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write(", ", "arlen: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[28:21]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write(", ", "arsize: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[20:18]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write(", ", "arburst: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[17:16]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write(", ", "arlock: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[15]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write(", ", "arcache: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[14:11]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write(", ", "arprot: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[10:8]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write(", ", "arqos: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[7:4]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write(", ", "arregion: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[3:0]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write(", ", "aruser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  1'b0)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	begin
	  v__h2241 = $stime;
	  #0;
	end
    v__h2235 = v__h2241 / 32'd10;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$display("%0d: ERROR: UART.rl_process_rd_req: unrecognized addr",
		 v__h2235);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("AXI4_Rd_Addr { ", "arid: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[99:93]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write(", ", "araddr: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[92:29]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write(", ", "arlen: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[28:21]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write(", ", "arsize: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[20:18]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write(", ", "arburst: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[17:16]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write(", ", "arlock: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[15]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write(", ", "arcache: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[14:11]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write(", ", "arprot: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[10:8]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write(", ", "arqos: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[7:4]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write(", ", "arregion: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[3:0]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write(", ", "aruser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] ==
	  2'd0 &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[7:5] !=
	  3'd0)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	begin
	  v__h2061 = $stime;
	  #0;
	end
    v__h2055 = v__h2061 / 32'd10;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$display("%0d: ERROR: UART.rl_process_rd_req: misaligned addr",
		 v__h2055);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("AXI4_Rd_Addr { ", "arid: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[99:93]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write(", ", "araddr: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[92:29]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write(", ", "arlen: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[28:21]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write(", ", "arsize: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[20:18]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write(", ", "arburst: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[17:16]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write(", ", "arlock: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[15]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write(", ", "arcache: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[14:11]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write(", ", "arprot: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[10:8]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write(", ", "arqos: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[7:4]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write(", ", "arregion: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[3:0]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write(", ", "aruser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  slave_xactor_f_rd_addr_first__4_BITS_92_TO_29__ETC___d17[1:0] !=
	  2'd0)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	begin
	  v__h3121 = $stime;
	  #0;
	end
    v__h3115 = v__h3121 / 32'd10;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$display("%0d: UART.rl_process_rd_req", v__h3115);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("AXI4_Rd_Addr { ", "arid: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[99:93]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "araddr: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[92:29]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "arlen: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[28:21]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "arsize: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[20:18]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "arburst: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[17:16]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "arlock: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[15]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "arcache: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[14:11]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "arprot: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[10:8]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "arqos: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[7:4]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "arregion: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[3:0]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "aruser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("AXI4_Rd_Data { ", "rid: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_rd_addr$D_OUT[99:93]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "rdata: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", rdr_rdata__h2901);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "rresp: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", rdr_rresp__h2902);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "rlast: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("True");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "ruser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_rd_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	begin
	  v__h3495 = $stime;
	  #0;
	end
    v__h3489 = v__h3495 / 32'd10;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$display("%0d: ERROR: UART.rl_process_wr_req: misaligned addr",
		 v__h3489);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("AXI4_Wr_Addr { ", "awid: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[99:93]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "awaddr: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[92:29]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "awlen: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[28:21]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "awsize: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[20:18]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "awburst: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[17:16]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "awlock: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[15]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "awcache: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[14:11]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "awprot: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[10:8]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "awqos: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[7:4]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "awregion: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[3:0]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "awuser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("AXI4_Wr_Data { ", "wdata: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_data$D_OUT[72:9]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "wstrb: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", slave_xactor_f_wr_data$D_OUT[8:1]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "wlast: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]) &&
	  slave_xactor_f_wr_data$D_OUT[0])
	$write("True");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]) &&
	  !slave_xactor_f_wr_data$D_OUT[0])
	$write("False");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write(", ", "wuser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] !=
	   2'd0 ||
	   !IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0]))
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	begin
	  v__h4389 = $stime;
	  #0;
	end
    v__h4383 = v__h4389 / 32'd10;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$display("%0d: ERROR: UART.rl_process_wr_req: unrecognized addr",
		 v__h4383);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("AXI4_Wr_Addr { ", "awid: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[99:93]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "awaddr: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[92:29]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "awlen: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[28:21]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "awsize: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[20:18]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "awburst: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[17:16]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "awlock: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[15]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "awcache: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[14:11]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "awprot: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[10:8]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "awqos: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[7:4]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "awregion: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[3:0]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "awuser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("AXI4_Wr_Data { ", "wdata: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_data$D_OUT[72:9]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "wstrb: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", slave_xactor_f_wr_data$D_OUT[8:1]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "wlast: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	  3'd0 &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	   3'h0 ||
	   rg_lcr[7]) &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	   3'h0 ||
	   !rg_lcr[7]) &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	   3'h1 ||
	   rg_lcr[7]) &&
	  NOT_IF_slave_xactor_f_wr_data_first__44_BITS_8_ETC___d254)
	$write("True");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] ==
	  3'd0 &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	   3'h0 ||
	   rg_lcr[7]) &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	   3'h0 ||
	   !rg_lcr[7]) &&
	  (IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[4:2] !=
	   3'h1 ||
	   rg_lcr[7]) &&
	  NOT_IF_slave_xactor_f_wr_data_first__44_BITS_8_ETC___d265)
	$write("False");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write(", ", "wuser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  1'b0)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	begin
	  v__h3725 = $stime;
	  #0;
	end
    v__h3719 = v__h3725 / 32'd10;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$display("%0d: ERROR: UART.rl_process_wr_req: unrecognized addr",
		 v__h3719);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("AXI4_Wr_Addr { ", "awid: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[99:93]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "awaddr: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[92:29]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "awlen: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[28:21]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "awsize: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[20:18]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "awburst: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[17:16]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "awlock: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[15]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "awcache: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[14:11]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "awprot: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[10:8]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "awqos: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[7:4]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "awregion: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[3:0]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "awuser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("AXI4_Wr_Data { ", "wdata: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_data$D_OUT[72:9]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "wstrb: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", slave_xactor_f_wr_data$D_OUT[8:1]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "wlast: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0 &&
	  slave_xactor_f_wr_data$D_OUT[0])
	$write("True");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0 &&
	  !slave_xactor_f_wr_data$D_OUT[0])
	$write("False");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write(", ", "wuser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[1:0] ==
	  2'd0 &&
	  IF_slave_xactor_f_wr_dataD_OUT_BITS_8_TO_5_EQ_ETC__q1[0] &&
	  IF_slave_xactor_f_wr_data_first__44_BITS_8_TO__ETC___d153[7:5] !=
	  3'd0)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	begin
	  v__h4708 = $stime;
	  #0;
	end
    v__h4702 = v__h4708 / 32'd10;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$display("%0d: UART.rl_process_wr_req", v__h4702);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("AXI4_Wr_Addr { ", "awid: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[99:93]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "awaddr: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[92:29]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "awlen: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[28:21]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "awsize: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[20:18]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "awburst: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[17:16]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "awlock: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[15]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "awcache: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[14:11]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "awprot: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[10:8]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "awqos: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[7:4]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "awregion: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[3:0]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "awuser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("AXI4_Wr_Data { ", "wdata: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_data$D_OUT[72:9]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "wstrb: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_data$D_OUT[8:1]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "wlast: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138 &&
	  slave_xactor_f_wr_data$D_OUT[0])
	$write("True");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138 &&
	  !slave_xactor_f_wr_data$D_OUT[0])
	$write("False");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "wuser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("\n");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("            ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("AXI4_Wr_Resp { ", "bid: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", slave_xactor_f_wr_addr$D_OUT[99:93]);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "bresp: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", v__h3375);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write(", ", "buser: ");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("'h%h", 1'd0, " }");
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_process_wr_req &&
	  NOT_cfg_verbosity_read_ULE_1_37___d138)
	$write("\n");
    if (EN_set_addr_map && set_addr_map_addr_base[2:0] != 3'd0)
      begin
        v__h5175 = $stime;
	#0;
      end
    v__h5169 = v__h5175 / 32'd10;
    if (EN_set_addr_map && set_addr_map_addr_base[2:0] != 3'd0)
      $display("%0d: WARNING: UART.set_addr_map: addr_base 0x%0h is not 8-Byte-aligned",
	       v__h5169,
	       set_addr_map_addr_base);
    if (EN_set_addr_map && set_addr_map_addr_lim[2:0] != 3'd0)
      begin
        v__h5281 = $stime;
	#0;
      end
    v__h5275 = v__h5281 / 32'd10;
    if (EN_set_addr_map && set_addr_map_addr_lim[2:0] != 3'd0)
      $display("%0d: WARNING: UART.set_addr_map: addr_lim 0x%0h is not 8-Byte-aligned",
	       v__h5275,
	       set_addr_map_addr_lim);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_receive && NOT_cfg_verbosity_read_ULE_1_37___d138)
	$display("UART_Model.rl_receive: received char 0x%0h; new_lsr = 0x%0h",
		 f_from_console$D_OUT,
		 new_lsr__h4946);
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_reset && cfg_verbosity != 8'd0)
	begin
	  v__h1852 = $stime;
	  #0;
	end
    v__h1846 = v__h1852 / 32'd10;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_reset && cfg_verbosity != 8'd0)
	$display("%0d: UART.rl_reset", v__h1846);
  end
  // synopsys translate_on
endmodule  // mkUART

